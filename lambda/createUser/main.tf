...
data "archive_file" "lambda" {
  type        = "zip"
  source_file = "./create_user_lambda.py"
  output_path = "./create_user_lambda.zip"
}

resource "aws_lambda_function" "CreateUserHandler" {
  function_name = "CreateUsersHandler"
  filename = "./create_user_lambda.zip"
...
resource "aws_cloudwatch_log_group" "usersLambda" {
  name = "/aws/lambda/${aws_lambda_function.CreateUserHandler.function_name}"
